...
const getSingleProductForecast: NextApiHandler = async (request, response) => {
  ...
  axios
    .get(
      `${process.env.SHELFCLOUD_SERVER_URL}/api/reorderingPoints/getForecastSingle?region=${request.query.region}&businessId=${request.query.businessId}&sku=${request.query.sku}`,
      {
        headers: {
