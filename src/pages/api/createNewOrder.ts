...
const createNewOrder: NextApiHandler = async (request, response) => {
  ...
  axios
    .post(`${process.env.API_DOMAIN_SERVICES}/${request.query.region}/api/createNewOrderShopify-V25-04.php?businessId=${request.query.businessId}`, {
      orderInfo: request.body.orderInfo,
      isPickUpOrder: request.body.isPickUpOrder,
    })
    .then(({ data }) => {
      response.json(data)
    })
