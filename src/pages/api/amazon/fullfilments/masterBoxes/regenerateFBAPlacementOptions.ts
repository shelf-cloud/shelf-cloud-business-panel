...
const createInboundPlan: NextApiHandler = async (request, response) => {
  ...
  axios
    .get(
      `${process.env.SHELFCLOUD_SERVER_URL}/api/amz_repair_workflows/regenerateFBAPlacementOptions/${request.query.region}/${request.query.businessId}/${request.query.inboundPlanId}`
    )
    .then(({ data }) => {
      response.json(data)
    })
