...
const addAuthSeller: NextApiHandler = async (request, response) => {
  ...
  axios
    ...
    .then(async ({ data }) => {
      axios
        .post(`${process.env.SHELFCLOUD_SERVER_URL}/amazon/sellers/reAuthSeller`, {
        ...
        .then(() => {
          axios
            .get(`${process.env.API_DOMAIN_SERVICES}/${request.query.region}/api/amazon/reAuthAmazonSeller.php?businessId=${request.query.businessId}`)
            .then(({ data }) => {
              response.json(data)
            })
            .catch((error) => {
