...
const InboundPlanDetails = ({ session, sessionToken }: Props) => {
  ...
  const [watingRepsonse, setWatingRepsonse] = useState<WaitingReponses>({
    ...
    generationTransportationOptions: false,
    boxLabels: false,
    printingLabel: false,
  ...
  const handleNextShipping = async (inboundPlanId: string) => {
    if (inboundPlanDetails!.placementOptions?.length > 0) {
      tabChange('2')
      return
    }
    setWatingRepsonse((prev: any) => ({ ...prev, shipping: true }))

    const cancelInboundPlanToast = toast.loading('Generating Placement Options...')
    try {
      const controller = new AbortController()
      const signal = controller.signal
  ...
  const handlePlacementExpired = async (inboundPlanId: string) => {
    setWatingRepsonse((prev: any) => ({ ...prev, shippingExpired: true }))
    const cancelInboundPlanToast = toast.loading('Generating Placement Options...')
    try {
      const controller = new AbortController()
      const signal = controller.signal
  ...
  const handlePrintShipmentLabel = async (shipmentId: string) => {
    ...
    try {
      ...
      const signal = controller.signal
      const response = (await axios
        .get(`${process.env.NEXT_PUBLIC_SHELFCLOUD_SERVER_URL}/api/amz_workflow/getLabels/${state.currentRegion}/${state.user.businessId}/${shipmentId}`, {
          signal,
          headers: {
            Authorization: `Bearer ${sessionToken}`,
          },
                        ...
                        <TabContent activeTab={activeTab} className='pt-2 pb-4 border-bottom'>
                          ...
                          <TabPane tabId='3'>
                            {inboundPlanDetails.steps[2].complete && (
                              <BoxLabels inboundPlan={inboundPlanDetails} handleNextStep={handlePrintShipmentLabel} watingRepsonse={watingRepsonse} />
                            )}
                          </TabPane>
                          <TabPane tabId='4'>{inboundPlanDetails.steps[3].complete && <></>}</TabPane>
                          <TabPane tabId='5'>{inboundPlanDetails.steps[4].complete && <TrackingDetails inboundPlan={inboundPlanDetails} />}</TabPane>
                        </TabContent>
                      </Col>
                    </Row>
                  </CardBody>
