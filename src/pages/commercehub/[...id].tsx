...
const CheckNumberDetails = ({ session }: Props) => {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      selector: (row: Invoice) => row.closedDate && <span className='fs-7'>{moment.utc(row.closedDate).local().format('D MMM YYYY')}</span>,
      sortable: false,
      center: true,
      compact: true,
    ...
    },
    {
      ...
      selector: (row: Invoice) => row.checkTotal > 0 && <span className='fs-7'>{row.invoiceTotal ? FormatCurrency(state.currentRegion, row.invoiceTotal) : ''}</span>,
      sortable: false,
      center: true,
      compact: true,
    },
    {
      ...
      selector: (row: Invoice) => row.closedDate && <span className='fs-7'>{moment.utc(row.closedDate).local().add(row.payterms, 'days').format('D MMM YYYY')}</span>,
      sortable: false,
      center: true,
      compact: true,
