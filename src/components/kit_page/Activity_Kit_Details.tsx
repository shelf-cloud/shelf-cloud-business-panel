            ...
            latestOrders
              ...
              .map((order) => (
                <tr key={order.orderNumber} className='border-t border-[color:var(--border)]'>
                  ...
                  <td className={'px-2 py-1 text-center ' + (order.isReceiving ? 'text-success' : 'text-danger')}>{order.isReceiving ? `+${order.qty}` : `-${order.qty}`}</td>
                </tr>
              ))
          ) : (
            <tr>
...
export default Activity_Kit_Details
