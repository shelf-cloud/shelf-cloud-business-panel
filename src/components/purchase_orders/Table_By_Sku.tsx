  ...
  const columns: any = [
    ...
    },
    {
      ...
      selector: (row: PurchaseOrderBySkus) => {
        return (
          <Link href={`/product/${row.inventoryId}/${row.sku}`} target='blank' rel='noopener noreferrer' className='!text-black'>
            <span className='text-[11.2px]'>{row.sku}</span>
          </Link>
        )
      },
      sortable: true,
    ...
    {
      name: <span className='font-extrabold text-[13px]'>Ordered</span>,
      selector: (row: PurchaseOrderBySkus) => {
    ...
    {
      name: <span className='font-extrabold text-[13px]'>Inbound</span>,
      selector: (row: PurchaseOrderBySkus) => {
        const totalSkuOrdered = row.orders.reduce((totalOrdered: number, order: PurchaseOrder) => {
          const totalSku = order.poItems.reduce((subtotal: number, item: PurchaseOrderItem) => {
            if (item.sku == row.sku) {
    ...
    {
      name: <span className='font-extrabold text-[13px]'>Arrived</span>,
      selector: (row: PurchaseOrderBySkus) => {
        const totalSkuOrdered = row.orders.reduce((totalOrdered: number, order: PurchaseOrder) => {
          const totalSku = order.poItems.reduce((subtotal: number, item: PurchaseOrderItem) => {
            if (item.sku == row.sku) {
