...
const MKP_table = ({ products, isLoading, handleOtherCosts, handleProposedPrice, handleSetSingleMargin, handleSetProductMargin, handleNotes }: Props) => {
  ...
  const columns: any = [
    {
      name: <span className='font-semibold text-center text-[11.2px]'>Image</span>,
    ...
    {
      name: (
        <span className='font-semibold text-center text-[11.2px]'>
          SKU
          <br />
          Title
        </span>
          ...
          <>
            <div className='flex flex-row justify-start items-center'>
              <Link href={`/product/${row.inventoryId}/${row.sku}`} target='_blank'>
                <p className='m-0 text-[11.2px] font-semibold'>{row.sku}</p>
              </Link>
              <CopyTextToClipboard text={row.sku} label='SKU' />
            </div>
            <p className='text-[11.2px] m-0 font-semibold text-black'>{row.title}</p>
          </>
        )
      },
      sortable: true,
    ...
    {
      name: <span className='font-semibold text-center text-[11.2px]'>ASIN</span>,
      selector: (row: MKP_Product) => {
        return row.asin !== '' ? (
