...
const DownloadPackingSlip = ({ order }: Props) => {
  ...
  const buildTemplate = async (logo: ArrayBuffer, signature: ArrayBuffer) => {
    ...
    worksheet.getCell('A14').value = order.orderNumber
    ...
    for (const item of order.orderItems) {
      ...
      const rowValues = [item.sku, item.name, '', '', item.quantity]
    ...
    const totalRow = worksheet.addRow(['', '', '', 'Total', totalItems])
    ...
    worksheet.getCell(`A${authorizeNumber}`).value = `Tracking Number: ${order.trackingNumber}`
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      a.href = url
      a.download = `Packing Slip - ${order.orderNumber}.xlsx`
      a.click()
    })
  }
  const downloadPackingSlip = async () => {
    const [logo, signature] = await Promise.all([fetchAssetAsArrayBuffer(shelf_cloud_blue_h), fetchAssetAsArrayBuffer(shelf_cloud_signature)])
