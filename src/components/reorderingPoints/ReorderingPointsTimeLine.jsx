...
const ReorderingPointsTimeLine = ({ productTimeLine, leadtime, daysRemaining, poDates, forecast, bestModel }) => {
  ...
  const dailytimeForecast = forecast
    ? Object.entries(forecast)?.reduce(function (result, [date, unitsSold]) {
  ...
  const series = [
    {
      name: 'Units Sold',
      data: Object.entries(timeLineSorted)
        .slice(0, currentDateIndex + 1)
        .map(([date, item]) => {
          return { x: date, y: item.unitsSold }
        }),
    },
    {
      name: 'Forecast',
      data: Object.entries(forecastSorted).map(([date, item]) => {
    ...
    },
    {
      ...
      data: Object.entries(timeLineSorted)
        .slice(0, currentDateIndex + 1)
        .map(([date, item]) => {
          return { x: date, y: item.dailyStock + item.dailyStockFBA }
        }),
    },
  ]
