...
const DownloadProductMD = ({ product }: Props) => {
  const markdown = useMemo(() => {
    ...
    return [
      ...
      `- FBA Qty: ${product.fbaQty + product.awdQty + product.awdInboundQty}`,
      ``,
      `- Total Inventory: ${product.warehouseQty + product.fbaQty + product.productionQty + product.receiving + product.awdQty + product.awdInboundQty}`,
      ``,
      `Important: Warehouse can only receive from warehouse inbound and production. FBA is separate and cannot be used to fulfill warehouse demand. FBA can receive from Warehouse.`,
      ``,
      `## Total Units Sold`,
      totalUnitsSold || '- None',
      ``,
