...
const SelectLTLFreightReadyDate = ({ shipmentId, selectedLTLTransportationOption, setfinalShippingCharges, transportationOptions }: Props) => {
  ...
  const carrierOptions = useMemo(() => {
    ...
    transportationOptions.forEach((option) => {
      if (!uniqueCarriers.has(option.carrier.alphaCode)) {
        uniqueCarriers.set(option.carrier.alphaCode, {
          value: option.carrier.alphaCode,
          label: option.carrier.name,
        })
      }
    })
  ...
  return (
    <div>
      ...
      <div className='w-100 gap-2' style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', alignContent: 'center', justifyItems: 'center' }}>
        {carrierTransportationOptions.map((option, index) => (
          <div
            key={index}
            className={
              'fs-7 py-1 px-2 border border-2 rounded-3' + (selectedLTLTransportationOption.transportationOptionId === option.transportationOptionId ? ' border-primary shadow-lg' : '')
            }
            style={{ cursor: 'pointer' }}
            onClick={() => handleChangeFreightReadyDate(option.transportationOptionId)}>
            <p className='m-0 p-0 fw-semibold'>{moment(option.carrierAppointment?.startTime).format('ddd')}</p>
            <p className='m-0 p-0 fw-semibold'>{moment(option.carrierAppointment?.startTime).format('MMM D')}</p>
            <p className='m-0 p-0'>
      ...
      <div className='mt-3 mb-2 d-flex flex-row justify-content-between align-items-center gap-3'>
        ...
        <Select className='fs-7' value={selectedCarrier} onChange={handleChangeCarrier} options={carrierOptions} />
      </div>
