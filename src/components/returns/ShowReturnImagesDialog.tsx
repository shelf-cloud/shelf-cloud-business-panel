import { UnsellablesType } from '@/types/returns/unsellables'
...
type Props = {
  imagesDialogItem: UnsellablesType | null
...
const ShowReturnImagesDialog = ({ imagesDialogItem, imagesDialogImages, handleImagesDialogOpenChange, setSelectedImage }: Props) => {
  return (
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <p className='mb-0! font-bold'>{imagesDialogItem?.barcode}</p>
          </DialogDescription>
        </DialogHeader>
          ...
          <div className='grid grid-cols-5 gap-2'>
            {imagesDialogImages.map((image, index) => {
              ...
              return (
                <Button
                  variant='outline'
                  key={`${imagesDialogItem?.barcode}-${image}-${index}`}
                  type='button'
                  className='focus-visible:ring-ring relative aspect-square size-16 overflow-hidden rounded-md border p-0 focus-visible:ring-2 focus-visible:outline-none'
...
export default ShowReturnImagesDialog
