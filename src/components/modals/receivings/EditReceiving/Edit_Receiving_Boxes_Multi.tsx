          ...
          <Row className='d-flex flex-row justify-content-around align-items-end gap-0'>
            <Col md={4}>
              <SelectSingleFilter
                ...
                selected={(() => {
                  const item = orderItems.find((item) => item.inventoryId == addSkuToBox.inventoryId && item.poId! == parseInt(addSkuToBox.poId))!
                  return item ? { value: item.inventoryId!, label: item.sku, description: `PO: ${item.poNumber} // ${addSkuToBox.poId}` } : { value: '', label: 'Choose SKU...' }
                })()}
                options={(() => {
                  ...
                  orderItems
                    ...
                    .forEach((item) => {
                      ...
                      skuList.push({
                        ...
                        label: item.sku,
                        description: `PO: ${item.poNumber} // ${item.poId}`,
                      })
                    })
                ...
                handleSelect={(option: SelectSingleValueType) => {
                  ...
                  const item = orderItems.find((item) => item.inventoryId == option!.value && item.poId! == parseInt(poId!))!
                  setaddSkuToBox((prev) => ({
                    ...prev,
                    poId: poId!,
                    sku: item.sku,
