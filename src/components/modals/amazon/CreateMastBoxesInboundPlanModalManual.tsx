...
const createMastBoxesInboundPlanManualSchema = z.object({
  inboundPlanName: z
    .string()
...
type CreateMastBoxesInboundPlanManualForm = z.infer<typeof createMastBoxesInboundPlanManualSchema>
...
const CreateMastBoxesInboundPlanModalManual = ({ orderProducts, showCreateInboundPlanModal, setShowCreateInboundPlanModal }: Props) => {
  ...
  const validation = useForm<CreateMastBoxesInboundPlanManualForm>({
    resolver: zodResolver(createMastBoxesInboundPlanManualSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: CreateMastBoxesInboundPlanManualForm) => {
    const validationResult = validateFulfillmentWarehouseUsage(orderProducts, 'master-boxes')
      ...
      const response = await axios.post(`/api/amazon/fullfilments/masterBoxes/createManualInboundPlan?region=${state.currentRegion}&businessId=${state.user.businessId}`, {
  ...
  return (
    <Dialog
      open={!!showCreateInboundPlanModal}
      onOpenChange={(open) => {
        if (!open) setShowCreateInboundPlanModal(false)
      }}>
      <DialogContent aria-describedby={undefined} className='max-h-[90vh] overflow-y-auto sm:!max-w-5xl' id='createInboundPlanModal'>
        <DialogHeader className='pr-6' id='myModalLabel'>
          <DialogTitle>
            <p className='text-[19.5px] m-0'>Create Manual Fulfillment - Send To Amazon</p>
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleCreateInboundPlan}>
          <div className='flex flex-wrap -mx-3'>
            <p className='text-[16.25px] font-extrabold text-primary'>Fulfillment Details</p>
            ...
            <div className='px-3 md:w-full'>
              <div className='text-right'>
                <Button disabled={loading} type='submit' variant='success'>
                  {loading ? <Spinner className='text-white' /> : 'Confirm Plan'}
                </Button>
              </div>
            </div>
          </div>
...
export default CreateMastBoxesInboundPlanModalManual
