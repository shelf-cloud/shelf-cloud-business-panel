...
const ExportBlankTemplate = ({ brands, suppliers, categories }: Props) => {
  const exportExcelFile = useCallback(async () => {
    ...
    const worker = new Worker(new URL('./exportBlankTemplateWorker.ts', import.meta.url), { type: 'module' })
    ...
    try {
      const buffer = await new Promise<ArrayBuffer>((resolve, reject) => {
        ...
        worker.postMessage({ brands, suppliers, categories })
      })
      ...
      anchor.href = url
      anchor.download = 'Empty Template.xlsx'
      anchor.click()
