...
const RPAIForecastDrawer = ({ product, isOpen, onClose, region }: Props) => {
  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => !open && onClose()} direction='right'>
      <DrawerPortal>
        {/* z-[1040] sits above Bootstrap navbar (z-1030) */}
        ...
        <DrawerHeader className='tw:border-b tw:border-border tw:pb-4'>
          <div className='tw:flex tw:flex-row tw:items-start tw:justify-between tw:gap-3'>
            <div className='tw:flex tw:flex-row tw:items-start tw:gap-3'>
              <div
                style={{ width: 40, height: 52, flexShrink: 0 }}
                className='tw:relative tw:overflow-hidden tw:rounded-md tw:border tw:border-border tw:bg-muted'>
                <img
                  loading='lazy'
              ...
              <div className='tw:flex tw:flex-col tw:gap-0.5 tw:min-w-0'>
                <DrawerTitle className='tw:text-base tw:text-primary tw:font-semibold tw:leading-tight'>
                  {product?.sku}
                </DrawerTitle>
                <DrawerDescription className='tw:text-sm tw:font-medium tw:text-foreground tw:line-clamp-2 tw:leading-snug'>
                  {product?.title}
                </DrawerDescription>
              </div>
            </div>
            <DrawerClose asChild>
              <button
                className='tw:shrink-0 tw:rounded-md tw:p-1.5 tw:text-muted-foreground tw:hover:bg-muted tw:hover:text-foreground tw:transition-colors'
                aria-label='Close'>
        ...
        <div className='tw:flex-1 tw:overflow-y-auto tw:px-4 tw:py-4'>
          <div className='tw:mb-4'>
            ...
            <p className='tw:text-xs tw:text-muted-foreground tw:m-0'>
              Comparison of all AI forecast models used for this product.
            </p>
          </div>
          ...
          {visibleModels.length === 0 ? (
            <div className='tw:flex tw:flex-col tw:items-center tw:justify-center tw:gap-2 tw:py-12 tw:text-center'>
              ...
              <p className='tw:text-sm tw:text-muted-foreground tw:m-0'>
                No AI forecast data available for this product.
              </p>
            </div>
          ) : (
